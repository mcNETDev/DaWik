...
import de.DaWik.DaWik.Inv.InventoryCraftingChest;
import de.DaWik.DaWik.Inv.InventoryCraftingChestResult;
...
public class ContainerSuperChest extends Container {
	...
	public ContainerSuperChest(InventoryPlayer invPlayer, TileSuperChest tile) {
		this.tile = tile;
	...
	public ItemStack transferStackInSlot(EntityPlayer player, int i) {
		...
		if ((slot != null) && slot.getHasStack()) {
			...
			if ((i < (tile.getSizeInventory() + 16)) && (i != 240))// Transferring
																	// from
																	// container
			{
				...
				slot.onSlotChange(itemstack1, itemstack);
			} else if (!BlockSuperChest.isStackValid(itemstack1) || !mergeItemStack(itemstack1, 0, tile.getSizeInventory(), false))// Transferring
																																	// from
																																	// player
			{
				return null;
	...
	public void addCraftingToCrafters(ICrafting iCrafting) {
		...
		// tile.getEnergyStored(ForgeDirection.DOWN)/32);
	...
	public void updateProgressBar(int id, int value) {
		if (id == 0) {
			tile.smeltingProgressTime = value;
		} else if (id == 1) {
			tile.energy.setEnergyStored(value * 32);
		} else if (id == 2) {
			tile.smeltingBurnSpeed = value;
		} else if (id == 3) {
			tile.smeltingAutoFeed = value;
		} else if (id == 4) {
			tile.lockOutputSlots = value == 1;
		}
	}

	public TileSuperChest getTile() {
		return tile;
	}
	...
	public class SlotDChest extends Slot {
		...
		@Override
		public boolean isItemValid(ItemStack stack) {
			return BlockSuperChest.isStackValid(stack);
		}
