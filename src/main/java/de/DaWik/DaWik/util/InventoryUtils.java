...
public class InventoryUtils {
	...
	public static void tryInsertStack(IInventory targetInventory, int slot, ItemStack stack, boolean canMerge) {
		if (targetInventory.isItemValidForSlot(slot, stack)) {
			...
			if (targetStack == null) {
				...
				if (targetInventory.isItemValidForSlot(slot, stack) && InventoryUtils.areMergeCandidates(stack, targetStack)) {
	...
	public static boolean areMergeCandidates(ItemStack source, ItemStack target) {
		return InventoryUtils.areItemAndTagEqual(source, target) && (target.stackSize < target.getMaxStackSize());
	}
	 ...
	 * passing false for doMove, it'll create a dummy inventory and its
	...
	public static int moveItemsFromOneOfSides(TileEntity te, IInventory inv, ItemStack filterStack, int maxAmount, int intoSlot, Set<ForgeDirection> sides) {
		...
		for (ForgeDirection dir : sides) {
			TileEntity tileOnSurface = InventoryUtils.getTileInDirection(te, dir);
			...
			if (tileOnSurface instanceof IInventory) {
				...
				if (filterStack == null) {
					slots = InventoryUtils.getAllSlots(neighbor);
				} else {
					slots = InventoryUtils.getSlotsWithStack(neighbor, filterStack);
				}
	...
	protected static void canInsert(ISidedInventory inventory, int slot, ForgeDirection side, ItemStack stack) {
		Preconditions.checkArgument(inventory.canInsertItem(slot, stack, side.ordinal()), "Item cannot be inserted into slot %s on side %s", slot, side);
	}

	protected static void canExtract(ISidedInventory inventory, int slot, ForgeDirection side, ItemStack stack) {
		Preconditions.checkArgument(inventory.canExtractItem(slot, stack, side.ordinal()), "Item cannot be extracted from slot %s on side %s", slot, side);
	}
