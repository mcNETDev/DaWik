...
import de.DaWik.DaWik.Blocks.BlockSuperChest;
...
import de.DaWik.DaWik.util.InventoryUtils;
import de.DaWik.DaWik.util.OreDoublingRegistry;

public class TileSuperChest extends TileEntity implements ISidedInventory, IEnergyReceiver, ICustomItemData {
	...
	public final int smeltingMaxBurnSpeed = 80;
	...
	public void updateEntity() {
		...
		if ((worldObj != null) && !worldObj.isRemote && (numUsingPlayers != 0) && (((ticksSinceSync + xCoord + yCoord + zCoord) % 200) == 0)) {
			...
			List<EntityPlayer> var2 = worldObj.getEntitiesWithinAABB(EntityPlayer.class,
					AxisAlignedBB.getBoundingBox(xCoord - var1, yCoord - var1, zCoord - var1, xCoord + 1 + var1, yCoord + 1 + var1, zCoord + 1 + var1));

			for (EntityPlayer var4 : var2) {
				if (var4.openContainer instanceof ContainerSuperChest) {
					++numUsingPlayers;
				}
			}
		}
		...
		if ((worldObj != null) && !worldObj.isRemote && (ticksSinceSync < 0)) {
			worldObj.addBlockEvent(xCoord, yCoord, zCoord, DaWikBlocks.superChest, 3, ((numUsingPlayers << 3) & 0xF8) | (facing & 0x7));
		}
	...
	public void updateEnergy() {// todo if no charging item wait a sec before
								// checking again
	...
	public void updateFurnace() {
		...
		// LogHelper.info("S.I.P: " + smeltInProgress + " S.S.D: " +
		// updateSuspended + " C.F.R: " + canFurnaceRun());
		if (updateSuspended && ((getTick() % 500) != 0)) {
			return;
		}
		...
		if (smeltInProgress) {
			// smeltingBurnSpeed = Math.min(energy.getEnergyStored() / 1000,
			// 100);
			smeltingBurnSpeed = 100;
	...
	private boolean canFurnaceRun() {
		...
		if (!flag) {
			return false; // Return false if there is nothing to smelt
		}
		// =====================================================================================
		// ===== Confirm that the furnace can still run when lock mode is
		// enabled ==============
		if (getLock()) {
			flag = false;
			for (int i = 0; i < 5; i++) {
				...
				if ((recipe == null) || (recipe.stackSize == 1)) {
					continue; // checks that there is more then one of at least
		...
		for (int i = 0; i < 5; i++) {
			...
			for (int j = 0; j < getSizeInventory(); j++) {
				// if (getStackInSlot(234 + j).stackSize >= getStackInSlot(234 +
				// j).getMaxStackSize()) continue;todo consider?
			...
			if (output.stackSize > 0) {
				return false; // return false if the output could not be added
								// to the inventory
			}
		}
		// =====================================================================================
	...
	private boolean trySmelt() {
		...
		do {
			for (int i = 0; i < 5; i++) {
				...
				if ((recipe == null) || (getLock() && (recipe.stackSize == 1))) {
					continue;
				}
				...
				if (result.stackSize > 0) {
					InventoryUtils.insertItemIntoInventory(this, result); // Insert
																			// stack
																			// into
																			// inventory
				}
				...
				if (result.stackSize == 0) {
					recipe.stackSize--;
					if (recipe.stackSize == 0) {
						setInventorySlotContents(234 + i, null);
					}
					itemsToProcess--;
					itemSmelted = true;
				}
	...
	public void feedNextItem() {
		...
		for (int i = 0; i < getSizeInventory(); i++) {
			...
			for (int j = 0; j < 5; j++) {
				...
				if ((getStackInSlot(234 + j) == null) || (candidate == null)) {
					continue;
				}
			...
			if (stacksFull[0] && stacksFull[1] && stacksFull[2] && stacksFull[3] && stacksFull[4]) {
				break;
			}
		}
	}
	...
	public String getInventoryName() {
		return hasCustomInventoryName() ? customName : StatCollector.translateToLocal(DaWikBlocks.superChest.getUnlocalizedName() + ".name");
	}
	...
	public void openInventory() {
		...
		numUsingPlayers++;
		worldObj.addBlockEvent(xCoord, yCoord, zCoord, DaWikBlocks.superChest, 1, numUsingPlayers);
	}
	...
	public boolean isItemValidForSlot(int i, ItemStack itemstack) {
		return BlockSuperChest.isStackValid(itemstack);
	}
